...
/**
 * Verify the entered code - ENHANCED DEBUG VERSION WITH SCREEN OUTPUT
 */
function verifyCode($user_id, $entered_code) {
    $conn = getDBConnection();
    
    // Add debug logging AND screen output
    $debug_msg = "DEBUG verifyCode: Starting verification for user_id: $user_id, code: $entered_code";
    error_log($debug_msg);
    echo "<div style='background: yellow; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
    ...
    if ($result->num_rows === 0) {
        $stmt->close();
        $debug_msg = "DEBUG verifyCode: No valid code found";
        error_log($debug_msg);
        echo "<div style='background: red; color: white; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
    ...
    $debug_msg = "DEBUG verifyCode: Found code - ID: {$verification['id']}, attempts: {$verification['attempts']}, expires: {$verification['expires_at']}";
    error_log($debug_msg);
    echo "<div style='background: lightblue; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
    ...
    if ($verification['attempts'] >= MAX_VERIFICATION_ATTEMPTS) {
        ...
        $debug_msg = "DEBUG verifyCode: Too many attempts, marking as used";
        error_log($debug_msg);
        echo "<div style='background: orange; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
    ...
    if ($verification['code'] === $entered_code) {
        ...
        $debug_msg = "DEBUG verifyCode: Code correct, verification successful";
        error_log($debug_msg);
        echo "<div style='background: green; color: white; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
        return ['success' => true, 'message' => 'Code verified successfully!'];
    } else {
        // Code is incorrect - increment attempts WITH ENHANCED DEBUG
        $new_attempts = $verification['attempts'] + 1;
        
        $debug_msg = "DEBUG verifyCode: Incorrect code. Incrementing attempts from {$verification['attempts']} to $new_attempts for ID {$verification['id']}";
        error_log($debug_msg);
        echo "<div style='background: pink; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
        ...
        if (!$update_attempts) {
            $debug_msg = "DEBUG verifyCode: Failed to prepare UPDATE statement: " . $conn->error;
            error_log($debug_msg);
            echo "<div style='background: red; color: white; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
        ...
        if (!$bind_result) {
            $debug_msg = "DEBUG verifyCode: Failed to bind parameters: " . $update_attempts->error;
            error_log($debug_msg);
            echo "<div style='background: red; color: white; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
        ...
        if (!$execute_result) {
            $debug_msg = "DEBUG verifyCode: Failed to execute UPDATE: " . $update_attempts->error;
            error_log($debug_msg);
            echo "<div style='background: red; color: white; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
            $update_attempts->close();
        ...
        $debug_msg = "DEBUG verifyCode: UPDATE executed successfully. Affected rows: $affected_rows";
        error_log($debug_msg);
        echo "<div style='background: lightgreen; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
        
        $update_attempts->close();
        
        if ($affected_rows === 0) {
            $debug_msg = "DEBUG verifyCode: WARNING - No rows were updated! Verification ID {$verification['id']} might be invalid";
            error_log($debug_msg);
            echo "<div style='background: red; color: white; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
            ...
            if ($check_result->num_rows > 0) {
                ...
                $debug_msg = "DEBUG verifyCode: Record still exists - ID: {$check_data['id']}, attempts: {$check_data['attempts']}, is_used: {$check_data['is_used']}";
                error_log($debug_msg);
                echo "<div style='background: orange; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
            } else {
                $debug_msg = "DEBUG verifyCode: Record with ID {$verification['id']} no longer exists!";
                error_log($debug_msg);
                echo "<div style='background: red; color: white; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
            }
            $check_stmt->close();
        ...
        $debug_msg = "DEBUG verifyCode: New attempts count: $new_attempts, remaining: $remaining_attempts";
        error_log($debug_msg);
        echo "<div style='background: lightcyan; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
        ...
        if ($remaining_attempts <= 0) {
            ...
            $debug_msg = "DEBUG verifyCode: Last attempt failed. Marking as used. Execute result: " . ($execute_mark ? 'SUCCESS' : 'FAILED') . ", Affected rows: $mark_affected";
            error_log($debug_msg);
            echo "<div style='background: orange; padding: 5px; margin: 5px; font-size: 11px;'>$debug_msg</div>";
